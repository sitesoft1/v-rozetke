...
class ModelExtensionModuleCdekIntegrator extends Model {
	...
	public function addDispatch($data = array()) {
		...
		if (!empty($data['orders'])) {
            ...
            //Здесь допишем уведомления для покупателей #################################
                
                //Получим настройки sdek для уведомлений
                $cdek_integrator_setting = $this->getSetting();
            
                if(!empty($cdek_integrator_setting) and !empty($order_info['dispatch_number'])){
    
                    $dispatch_number = $order_info['dispatch_number'];
                    ...
                    $sms_msg = $cdek_integrator_setting['sms_msg'];
                    $email_subject = $cdek_integrator_setting['email_subject'];
                    $email_msg = $cdek_integrator_setting['email_msg'];
                    $sms_send = $cdek_integrator_setting['sms_send'];
                    $email_send = $cdek_integrator_setting['email_send'];
                    //Запишем настройки sdek для уведомлений КОНЕЦ
    
                    //file_put_contents('cdek_integrator_setting.txt', print_r($cdek_integrator_setting, true));
                    //mail('sitesoft1@gmail.com', 'cdek_integrator_setting', print_r($cdek_integrator_setting, true));
    
                    //Получим настройки sdek для уведомлений КОНЕЦ
    
                    //Номер заказа и статус
                    $order_id = $order_info['order_id'];
                    $status_id = $order_info['status_id'];
    
                    //Если есть телефон отправим смс уведомление
                    $recipient_telephone = $order_info['recipient_telephone'];
                    if(isset($recipient_telephone) and !empty($recipient_telephone)){
                        if(isset($sms_api_key) and !empty($sms_api_key) and isset($sms_send) and !empty($sms_send)){
                            if(isset($sms_msg) and !empty($sms_msg)){
                                //Здесь отправляем смс
    
                                $sms_msg = str_replace('[order_id]', $order_id, $sms_msg);
                                $sms_msg = str_replace('[dispatch_number]', $dispatch_number, $sms_msg);
                                //file_put_contents('send_sms.txt', 'Отправили СМС на номер: '.$recipient_telephone . PHP_EOL . 'Сообщение: ' . $sms_msg);
                                //mail('sitesoft1@gmail.com', 'Отправили СМС на номер: '.$recipient_telephone, 'Отправили СМС на номер: '.$recipient_telephone . PHP_EOL . 'Сообщение: ' . $sms_msg);
                                ...
                                if($sms_api_login and $sms_api_password){
                                    ...
                                    //$sms_rezult = file_get_contents('https://smsc.ru/sys/send.php?login='.$sms_api_login.'&psw='.$sms_api_password.'&phones=+380671748435&mes=test');
                                    //file_put_contents('sms_rezult.txt', print_r($sms_rezult, true));
                                    //mail('sitesoft1@gmail.com', 'Результат отправки смс', print_r($sms_rezult, true));
                                }
                                //Шлем смс конец
                                
                                //Здесь отправляем смс КОНЕЦ
                            }
                        }
                    }
                    //Если есть телефон отправим смс уведомление КОНЕЦ
    
                    //Если есть есть почта отправим уведомление на email
                    $recipient_email = $order_info['recipient_email'];
                    if(isset($recipient_email) and !empty($recipient_email)){
                        if(isset($email_send) and !empty($email_send)){
                            if(isset($email_msg) and !empty($email_msg)){
                                //Отправляем email уведомление
    
                                    $email_headers = 'Content-type: text/html; charset=utf-8';
                                    
                $msg = '<div style="font-family:Arial,Helvetica,sans-serif;font-size:12px;color:#000000"><div style="width:680px">
<a href="https://v-rozetke.ru/" title="V-ROZETKE.RU" target="_blank"><img src="https://v-rozetke.ru/image/catalog/_розетке_исх.png" alt="V-ROZETKE.RU" style="margin-bottom:20px;border:none;max-width:250px;display:block;margin:20px auto" class="CToWUd"></a><p style="margin-top:40px;margin-bottom:20px;text-align:center;font-size:16px">';
                                    $email_msg = htmlspecialchars_decode($email_msg);
                                    $email_msg = str_replace('[order_id]', $order_id, $email_msg);
                                    $email_msg = str_replace('[dispatch_number]', $dispatch_number, $email_msg);
    
                                $msg .= $email_msg;
    
                                $msg .= '</p><div style="color:#757575;font-size:12px;text-align:center;margin:30px 0">г. Москва, Лучников переулок 4, строение 2, офис 19.
<div style="padding:15px 0"><a style="font-size:16px;line-height:24px;font-weight:bold;color:#333333" href="tel:8%20(495)%20642-54-58" target="_blank">8 (495) 642-54-58
</a><br><a style="font-size:16px;line-height:24px;font-weight:bold;color:#333333" href="tel:8%20(925)%20572-25-05" target="_blank">8 (925) 572-25-05
</a><br></div>			Пн-Пт: с 10 до 19
							Сб: выходной
							Вс: выходной
    <div class="yj6qo"></div><div style="padding:15px 0" class="adL"></div><div class="adL"></div></div></div></div>';
                                $email_subject = str_replace('[order_id]', $order_id, $email_subject);
                                    $email_subject = str_replace('[dispatch_number]', $dispatch_number, $email_subject);
                                    if(!$email_subject or empty($email_subject)){
                                        $email_subject = 'Ваш заказ отправлен';
                                    }
                                    mail($recipient_email, $email_subject, $msg, $email_headers);
                                    /*
									mail('balaban1990@ukr.net', $email_subject, $msg, $email_headers);
                                    mail('voronsglass@mail.ru', $email_subject, $msg, $email_headers);
                                    mail('dstarshov@mail.ru', $email_subject, $msg, $email_headers);
                                    mail('brechkoostap@yandex.ru', $email_subject, $msg, $email_headers);
                                    mail('9675003@gmail.com', $email_subject, $msg, $email_headers);
									*/
                                    //file_put_contents('send_email.txt', 'Отправили уведомление на email: '.$recipient_email.PHP_EOL.' Тема: '.$email_subject.PHP_EOL.' Сообщение: '.PHP_EOL.$email_msg);
                                //Отправляем email уведомление КОНЕЦ
                            }
                        }
                    }
                    //Если есть есть почта отправим уведомление на email КОНЕЦ
    
                    //Логи
                    //file_put_contents('dispatch_number.txt', $order_info['dispatch_number']);
                    //file_put_contents('order_info.txt', print_r($order_info, true));
                    //mail('sitesoft1@gmail.com', 'Номер отправления: '.$order_info['dispatch_number'], 'Номер отправления: '.$order_info['dispatch_number']. PHP_EOL . print_r($order_info, true));
    
                    
                }
                
                //return FALSE;//ZAKOMENTIT!!!
            //Здесь допишем уведомления для покупателей КОНЕЦ!!! #########################
