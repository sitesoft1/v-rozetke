...
use DvBusiness\ApiClient\DvBusinessApiClient;
use DvBusiness\ApiClient\DvBusinessApiHttpException;
...
class ControllerExtensionShippingDvBusiness extends Controller
{
    ...
    public function index()
    {
        ...
        $data['is_payment_method_select_enabled'] = !in_array(PaymentMethodEnum::PAYMENT_METHOD_NON_CASH, $allowedPaymentsMethod);
    ...
    public function orderFormCreate()
    {
        ...
        $dvBusinessApiClient = new DvBusinessApiClient($dvOptions->getApiUrl(), $dvOptions->getAuthToken());
        $postBody = file_get_contents('php://input');
        $data = json_decode($postBody, true) ?? [];
        ...
        } catch (DvBusinessApiHttpException $exception) {
    ...
    public function createAuthToken()
    {
        ...
        $dvBusinessApiClient = new DvBusinessApiClient($apiUrl);
        ...
        if ($authToken) {
            ...
            try {
                $dvBusinessApiClient = new DvBusinessApiClient($apiUrl, $authToken);
            ...
            } catch (DvBusinessApiHttpException $businessApiHttpException) {
    ...
    private function getPaymentTypes(): array
    {
        ...
        // Выберем доступные методы из списка не Карта (баланс и кеш)
        $paymentTypes = [];
        $intersectsPayments = array_intersect([PaymentMethodEnum::PAYMENT_METHOD_NON_CASH, PaymentMethodEnum::PAYMENT_METHOD_CASH], $allowedPaymentsMethod);
        if ($intersectsPayments) {
            $paymentTypes = [$this->language->get('dvbusiness_payment_type_' . array_shift($intersectsPayments))];
        }

        $bankCards = BankCardsManager::getShopBankCards($dvBusinessApiClient);

        foreach ($bankCards as $bankCard) {
            $paymentTypes[$bankCard->getBankCardId()] = $this->language->get('dvbusiness_payment_type_card') . ' ' . $bankCard->getBankCardNumberMask();
        }
